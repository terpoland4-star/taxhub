 ...
 * (`.hamadine/app-env.json`), which selects the dev user (see `use-current-user`);
...
const BEARER_KEY = "hamadine-auth.bearer-token";
...
/**
 * The sandbox live preview runs this app inside an iframe on a `*.hamadine-sandbox.com`
...
function inLivePreview(): boolean {
  return (
    ...
    window.location.hostname.endsWith(".hamadine-sandbox.com")
  );
}

/** Message the popup posts back to the opener once sign-in completes. */
type PopupMessage = { source: "hamadine-auth-popup"; token: string | null; error?: string };

/**
 * Start sign-in with one upstream provider (`providerId` from `HAMADINE_PROVIDERS`),
 * federating through the Hamadine auth broker.
 *
 * - **Live preview** (`*.hamadine-sandbox.com` iframe): opens a POPUP to
...
function openSignInPopup(providerId: string): Window | null {
  ...
  const name = `hamadine-signin-${Date.now()}`;
...
function waitForPopupToken(popup: Window): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.source !== "hamadine-auth-popup") return;

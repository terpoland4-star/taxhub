 ...
 * - `/__hamadine/manifest.webmanifest` → per-app-named manifest (kept out of
 *   public/ so this dynamic response is the only one).
 * - Other HTML documents → stream-inject PWA + OG head tags at `</head>`.
 *   OG identity is baked via `virtual:hamadine-og-identity` at `vite build`
...
import { hamadineOgIdentity } from "virtual:hamadine-og-identity";
import {
  acceptsHtml,
  createHeadInjector,
  isDocumentPath,
  ...
  renderWebManifest,
} from "../../scripts/hamadine-pwa-shared.mjs";

interface HamadinePwaEvent {
  url: URL;
  req: { method: string; headers: Headers };
}

function requestHost(event: HamadinePwaEvent): string {
  return (
...
export default async function hamadinePwaMiddleware(
  event: HamadinePwaEvent,
  ...
  if (path === "/__hamadine/manifest.webmanifest" || path === "/__hamadine/manifest.json") {
